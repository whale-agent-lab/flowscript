...
def main() -> int:
    parser = argparse.ArgumentParser(description="生成并分析员工与区域模拟 CSV。")
    ...
    try:
        ...
        if not selected_rows:
            raise ValueError("时间筛选后没有可分析的数据。")
        ...
        for value, group_rows in sorted(groups.items()):
            ...
            completed = sum(row["status"] == "已完成" for row in group_rows)
            clusters.append({
                "group_value": value,
        ...
        if len(selected_rows) < len(rows):
            warnings.append(f"时间筛选保留 {len(selected_rows)}/{len(rows)} 条记录。")
        write_json(status_output, {"state": "success", "message": "模拟 CSV、时间筛选、画像和聚类统计生成完成。", "warnings": warnings})
        return 0
    except (OSError, KeyError, TypeError, ValueError) as exc:
        message = f"模拟数据生成或画像失败：{exc}"
